defmodule KacyApi.MessageTest do
    ...
    describe "create/1" do
      test "successfully creates a message struct with valid params" do
        params = %{authorName: "Alice", content: "Hello, world!", signature: "Best regards"}
        message = Message.create(params)

        assert %Message{authorName: "Alice", content: "Hello, world!", signature: "Best regards"} = message
      end
    end

    describe "validate/1" do
      test "returns :ok for a valid message" do
        message = %Message{authorName: "Alice", content: "Hello, world!", signature: "Best regards"}
        assert {:ok, ^message} = Message.validate(message)
      end

      test "returns an error when authorName is blank" do
        message = %Message{authorName: "", content: "Hello, world!", signature: "Best regards"}
        assert {:error, "author name can't be blank!"} = Message.validate(message)
      end

      test "returns an error when content is blank" do
        message = %Message{authorName: "Alice", content: "", signature: "Best regards"}
      ...
      test "returns an error when authorName exceeds the maximum length" do
        message = %Message{authorName: String.duplicate("A", 41), content: "Hello, world!", signature: "Best regards"}
      ...
      test "returns an error when content exceeds the maximum length" do
        message = %Message{authorName: "Alice", content: String.duplicate("A", 2001), signature: "Best regards"}
